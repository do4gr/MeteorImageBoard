...
export default compose(graphql(updatePost, {name: "updatePost"}), graphql(upvotePost, {name: "upvotePostMutation"}), graphql(deletePost, {name: "deletePostMutation"}), graphql(downvotePost, {name: "downvotePostMutation"}), graphql(countQuery, {
  options: ownProps => ({
    variables: {
      id: ownProps.post.id
    }
