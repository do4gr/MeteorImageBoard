...
class PostUpload extends React.Component {
  ...
  isSubmittable() {
    return (
      (!this.props.enableDescription || this.state.description) &&
      (this.state.file ||
              ...
              <form className="" onSubmit={this.onPostClicked.bind(this)}>
                ...
                {this.props.enableMemeSelect && (
                  <span>
                    ...
                    <Button
                      type="button"
                      className="pa3 bn ttu pointer bg-black-10 dim btn-normal"
                      onClick={this.onSelectMeme.bind(this)}
                    >
                      Select Meme
                    </Button>
                  </span>
                ...
                {!this.state.imageUrl && (
                  <div className="w-100 dropzone mv3">
                    ...
                    !this.state.isDraggingFile && <span>Choose a picture</span>}
                    {this.state.isLoadingFile && (
                      <span>Processing File...</span>
                    )}
                    {this.state.isDraggingFile &&
                    this.state.isValidType && <span>Drop to Upload</span>}
                    {this.state.isDraggingFile &&
                    !this.state.isValidType && <span>Invalid File</span>}
                  </div>
                )}
                {this.state.imageUrl && (
                  <div
                    className={
                      'imagePreviewCotnainer w-100 mv3' +
                      (this.state.isDraggingFile ? ' isDragging' : '')
                    }
                  >
                    <div
                      className={
                        'imagePreview' +
                        (this.state.isTextEntered ? ' textEntered' : '')
                      }
                    >
                      <img
                      ...
                      />
                      {(this.props.enableImageText ||
                        this.props.enableMemeSelect) && (
                        <ContentEditable
                          onFocus={this.onImageTextFocused.bind(this)}
                          onBlur={this.onImageTextBlured.bind(this)}
                          className={
                            'outlined upper imageText uncheckedSpelling' +
                            (this.state.isTextEntered ? '' : ' placeholder')
                          }
                          html={this.state.upperImageText}
                          onChange={this.onImageTextChanged.bind(
                            this,
                            'upperImageText',
                          )}
                        />
                      ...
                      {(this.props.enableImageText ||
                        ...
                        <ContentEditable
                          ...
                          onBlur={this.onImageTextBlured.bind(this)}
                          className={
                            'outlined lower imageText uncheckedSpelling' +
                            (this.state.isTextEntered ? '' : ' placeholder')
                          }
                ...
                <Button
                  ...
                  className={
                    'pa3 bn ttu pointer' +
                    (this.isSubmittable()
                      ? ' bg-black-10 dim btn-normal'
                      : 'btn-normal black-30 bg-black-05 disabled')
                  }
                >
                  {this.state.isSubmitting ? this.state.isRendering ? (
                    'Rendering...'
                  ) : (
                    'Submitting ...'
                  ) : (
  ...
  recalcImageFontSize(element) {
    if (!element) {
      ...
      $(element).css({
        'font-size': $(element).width() * PostUpload.fontSizePercentage + 'px',
      })
    }
  }
  ...
  onImageLoaded(event) {
    var imageElement =
      event.nativeEvent.srcElement || event.nativeEvent.originalTarget
    this.recalcImageFontSize()
          ...
          this.drawText({
            ...
            this.drawText({
              ...
              ctx.drawImage(
                blackLowerText,
                -offset,
                height - blackLowerText.height - offset,
              )
              ctx.drawImage(
                blackLowerText,
                -offset,
                height - blackLowerText.height,
              )
              ctx.drawImage(
                blackLowerText,
                -offset,
  ...
  drawText(options) {
    ...
    frame.onload = () => {
      ...
      var upperStyle = this.styleObjectToString(PostUpload.textStyle, {
        'font-size': fontSize + 'px',
        width: width + 'px;',
        color: color,
        position: 'relative',
      })
      var lowerStyle = this.styleObjectToString(PostUpload.textStyle, {
        'font-size': fontSize + 'px',
        width: width + 'px;',
        color: color,
        position: 'relative',
